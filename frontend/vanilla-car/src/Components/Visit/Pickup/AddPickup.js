...
export class AddPickup extends Component {
    constructor(props) {
        super(props);
    ...
    async handleSubmit(e) {
        ...
        var before = time.toLocaleString("sv-SE", {hour12: false});
        var fixedbefore =
            before.replace(" ", "").slice(0, 10) +
            "T" +
            before.slice(10).replace(" ", "");

        e.preventDefault();
        if (this.state.PickupDateTime === "") {
            await this.setState({
                PickupDateTime: fixedbefore,
            });
        }
        if (this.state.PickupCity === "") {
            await this.setState({
                PickupCity: this.props.clientcity,
            });
        }
        if (this.state.PickupStreet === "") {
            await this.setState({
                PickupStreet: this.props.clientstreet,
            });
        }
        if (this.state.PickupPostalCode === "") {
            await this.setState({
                PickupPostalCode: this.props.clientpostalcode,
            });
        }
    ...
    render() {
        ...
        var min = start.toLocaleString("sv-SE", {hour12: false});
        var before = time.toLocaleString("sv-SE", {hour12: false});
